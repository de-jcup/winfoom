...
public class IpAddressUtils {
     ...
     * {@link InetAddress#getAllByName(String) InetAddress.getAllByName()}.
     *
     ...
     * or an IPv6 address (any form accepted by {@link Inet6Address}),
     ...
     * {@link InetAddress#getByName(String)} will block for
     ...
     *                              found.
     * @see #nameResolve(String, IpTypePreference)
     */
    public static InetAddress[] nameResolveArr(String host, IpTypePreference ipTypePref)
     ...
     * {@link #nameResolveArr(String, IpAddressUtils.IpTypePreference) nameResolveArr()}
     ...
     * @throws UnknownHostException if no IP address for the host could be
     *                              found.
     * @see #nameResolveArr(String, IpTypePreference)
     */
    public static InetAddress nameResolve(String host, IpTypePreference ipTypePref)
            throws InterruptedException, UnknownHostException, TimeoutException {
        InetAddress[] ipAddresses = nameResolveArr(host, ipTypePref);
     ...
     * @see #nameResolveArr(String, IpTypePreference)
     */
    public static InetAddress nameResolve(String host, int timeoutMs)
            throws InterruptedException, UnknownHostException, TimeoutException {
        return nameResolve(host, IpTypePreference.ANY_JDK_PREF);
    }
     ...
     * {@link InetAddress#getByName(String)} but this method
    ...
    public static boolean looksLikeIpv6Literal(String ipAddressStr) {
        ...
        if (ipAddressStr.length() >= 5) {
            return (ipAddressStr.charAt(3) == ':') || (ipAddressStr.charAt(4) == ':');
        }
        return false;
    }
     ...
     * false} if in doubt. Java allows "123" as a {@link Inet4Address
    ...
    public static boolean looksLikeIpv4Literal(String ipAddressStr) {
        ...
        if (dotPos > 0 && dotPos < ipAddressStr.length() - 1 && dotPos <= 3) {
            ...
            return isAsciiDigit(ipAddressStr.charAt(dotPos + 1));
        } else {
