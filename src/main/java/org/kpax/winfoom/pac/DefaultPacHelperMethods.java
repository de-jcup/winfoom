...
import org.kpax.winfoom.pac.net.LocalAddressUtils;
 ...
 * @author lbruun
 */
public class DefaultPacHelperMethods implements PacHelperMethods {

    private static final Logger LOGGER = Logger.getLogger(DefaultPacHelperMethods.class.getName());
    ...
    public boolean isResolvable(String host) {
        try {
            IpAddressUtils.nameResolve(host, IpAddressUtils.IpTypePreference.IPV4_ONLY);
            return true;
    ...
    public String dnsResolve(String host) {
        try {
            return IpAddressUtils.nameResolve(host, IpAddressUtils.IpTypePreference.IPV4_ONLY)
                    .getHostAddress();
    ...
    public boolean isInNet(String host, String pattern, String mask) {
        try {            
            String hostIP = IpAddressUtils.nameResolve(host, IpAddressUtils.IpTypePreference.IPV4_ONLY).getHostAddress();
    ...
    public boolean isResolvableEx(String host) {
        try {
            IpAddressUtils.nameResolve(host, IpAddressUtils.IpTypePreference.ANY_JDK_PREF)
                    .getHostAddress();
            return true;
        } catch (InterruptedException | UnknownHostException | TimeoutException ex) {
            return false;
    ...
    public String dnsResolveEx(String host) {
        try {
            return IpAddressUtils.nameResolve(host, IpAddressUtils.IpTypePreference.ANY_JDK_PREF)
                    .getHostAddress();
        } catch (InterruptedException | UnknownHostException | TimeoutException ex) {
            return "";
        }
    ...
    public boolean isInNetEx(String ipAddress, String ipPrefix) {
        ...
        InetAddress iAddr;
        try {
            iAddr = IpAddressUtils.nameResolve(ipAddress, IpAddressUtils.IpTypePreference.ANY_JDK_PREF);
        } catch (InterruptedException | UnknownHostException | TimeoutException ex) {
            return false;
        }
